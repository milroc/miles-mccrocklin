...
import { asJsonObject, asNumber, asString, asStringArray, isBoolean, isJsonObject, isNumber, isString } from '../src/utils/json';
  ...
  for (const f of files) {
    ...
    for (const line of text.split('\n')) {
      ...
      try {
        ...
        const eventFields = asJsonObject(o.fields);
...
function loadEntries(): ReviewEntry[] {
  ...
  const overlay = (id: string, base: Partial<ReviewEntry>): ReviewEntry => {
    ...
    // SAFETY: `base` carries the required id/src/source that
    // Partial<ReviewEntry> can't promise, because every caller below
    // builds it with them. The spreads only add optional fields.
    return {
      ...base,
      ...patch,
  ...
  for (const e of photoArr) {
    ...
    out.push(overlay(id, {
      ...
      theme: asStringArray(e.theme),
      species: asString(e.species),
      story: asString(e.story),
      entities: asStringArray(e.entities),
...
const server = serve({
  ...
  async fetch(req: Request): Promise<Response> {
    ...
    if (p.startsWith('/api/photo/') && req.method === 'PUT') {
      ...
      // SAFETY: this dev-only review server is called by the page it serves,
      // which posts a flat field map. Values are filtered through the guards
      // below before any of them reaches the label log.
      ...
      try {
        ...
        return new Response(`label log append failed: ${messageOf(e)}`, { status: 500 });
      }
