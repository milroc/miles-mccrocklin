...
import { asJsonObject, asNumber, asString, isJsonObject, isNumber, isString } from '../src/utils/json';
...
function loadEvents(tier: Tier, strict: boolean): LabelEvent[] {
  ...
  for (const f of listJsonl(dir)) {
    ...
    for (const line of text.split('\n')) {
      ...
      try {
        // SAFETY: JSON.parse returns any; JsonValue is simply the honest
        // type for it. The isJsonObject check on the next line does the
        // actual narrowing.
...
function buildBaseMap(): BaseMap {
  ...
  // SAFETY: photography.json is this repo's own event-log output,
  // regenerated by merge-labels.ts and committed alongside it. Each
  // row's fields are read through the JsonObject guards below rather
  // than trusted.
  const photoRaw = JSON.parse(readFileSync(PHOTOGRAPHY_JSON, 'utf8')) as Array<JsonObject>;
  for (const e of photoRaw) {
    const bareId = asString(e.id);
    const src = asString(e.src);
    if (!bareId || !src) continue;
    ...
    idToSrc.set(nsId, src);
    srcToId.set(src, nsId);
  }

  // me-* entries from me.json sabbatical-travel.
  // SAFETY: repo-owned me.json. JsonObject claims only that it parsed
  // to an object, which JSON.parse of a committed file guarantees; the
  // walk below narrows every node it touches.
  ...
  // SAFETY: photo-atlas.json is generated by build-photo-atlas.ts in
  // this repo and committed; AtlasEntry is the shape that script writes.
...
function setProseProvenance(entry: MergedEntry, field: string, tier: Tier): void {
  if (tier === 'ai' || tier === 'refined') {
    // SAFETY: prose_provenance is written by this same file as a flat
    // field -> tier map; MergedEntry's index signature widens it back to
    // JsonValue on the way out.
...
function clearProseProvenance(entry: MergedEntry, field: string): void {
  // SAFETY: as in setProvenance above — written here, read here.
...
export async function merge(opts: Partial<CliOptions> = {}): Promise<void> {
  ...
  if (!o.skipRefine) {
    ...
    const hasDupeCandidates = humanEvents.some((ev) =>
      inScope(ev.id) &&
      (asString(ev.fields.dupeOf)?.length ?? 0) > 0,
    );
    ...
    if (hasNoteCandidates || hasDupeCandidates) {
      ...
      try {
        ...
        console.warn(
          `merge-labels: refinement + dupe-merge skipped — ${messageOf(e)}\n` +
    ...
    if (hasNoteCandidates && textModel && backend) {
      try {
        ...
        console.warn(`merge-labels: refinement failed — ${messageOf(e)}`);
      }
    ...
    if (hasDupeCandidates && textModel && backend) try {
      ...
      console.warn(
        `merge-labels: dupe-merge skipped — ${messageOf(e)}\n` +
  ...
  if (onlyScope) {
    ...
    if (photoOnly.size > 0) {
      ...
      for (let i = 0; i < existing.length; i++) {
        // SAFETY: the loop index came from existing.entries(), and every row
        // in photography.json carries a string id — ingest-photography.ts
        // refuses to write one without.
    ...
    if (meAtlasOnly.size > 0) {
      // SAFETY: photo-classifications.json is merge-labels.ts output,
      // committed alongside the code that writes it.
