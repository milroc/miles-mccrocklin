...
function parseArgs(argv: string[]): CliOptions {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    switch (a) {
      ...
      case '--source':
        // SAFETY: validated below against SOURCE_FILTERS, which exits
        // with usage on anything else.
        opts.source = argv[++i] as SourceFilter;
        break;
...
async function main(): Promise<void> {
  ...
  for (const job of queue) {
    ...
    try {
      ...
      failed += 1;
      console.warn(`  [${job.id}] FAILED: ${messageOf(e)}`);
    }
  }
