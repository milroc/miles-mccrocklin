...
| 2026-08-04 | /builder/ era, job, and sabbatical prose moved to roman (`BuilderProse` + host-scoped lede override) | On /builder/ the `.entry-summary` italic IS the body content — the italic register isn't meant for sustained reading. The sabbatical lede joins the roman register on /builder/ only. Italic unchanged everywhere else (/resume/ summaries and ledes, builder Summary lede, taglines). Owner-approved. |
| 2026-08-04 | /builder/ footer explorer door carries the live tile globe | Splash tile mode (tile topology + tile texture variants), mounted only after window load + an idle slot so the document always wins the bandwidth race; WireGlobe is the base and perf-fallback state. The header tagline frame was dropped as redundant with the Summary. Owner-approved. |
